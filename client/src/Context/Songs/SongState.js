...
const SongState = (props) => {
  ...
  let oldReleaseFunc = (range) => {
    let newList = songList.filter((curr) => {
      let date = new Date();
      return date.getFullYear() - Number(curr.releaseDate) > 20;
    });
    newList = newList.sort((a, b) => a.title.localeCompare(b.title));
    let length = range ? range : newList.length;
    newList = newList.slice(0, length);
    return newList;
  };
